...
**************************************************************************/?>
    ...
    <h3 style="float:right"><a href="logout.php">Logout <?php //session_start(); 
echo $_SESSION['name'];
?></a></h3>
	<?php 
	/* Schedule count*/
...
$con=mysqli_connect('localhost', 'CrooksC', 'lulls.dug', 'CrooksC'); 
	$q = "SELECT * from  schedule_count where sc_id = 2";
	...
	/* Schedule count End*/
	   ...
	   date_default_timezone_set('America/New_York');
		//error_reporting(1);
		getTimes();
		displaySchedule($c,$lno,$sc_count); 
		
		
		
		function displaySchedule(&$c,&$lno,&$sc_count)
		{
	...
	$con=mysqli_connect('localhost', 'CrooksC', 'lulls.dug', 'CrooksC'); 
					...
					//{
						$q = "SELECT * from sc_user where user_id = 1 and schedule_id = 2";
						$r = mysqli_query($con, $q);
	...
	$con=mysqli_connect('localhost', 'CrooksC', 'lulls.dug', 'CrooksC'); 
			...
			$query = "SELECT * from schedule where user_id = 1 and schedule_id = 2";
			$r = mysqli_query($con,$query);
			while($row = mysql_fetch_array($r,MYSQLI_BOTH))
			{
				$s = $row['sc_dt'];
